...
#[test]
fn set_create_from_slice() {
    // Java: `EconomicSet.create(Iterable<E> c)` (added in 25.1) creates a set
    // and `addAll(c)`s all elements. The slice variant is the Rust analog.
    let dst = BTreeEconomicSet::<i32>::create_from_slice(&[3, 1, 2, 1]);
    assert_eq!(dst.size(), 3, "duplicates should be deduplicated");
    assert!(dst.contains(&1));
    assert!(dst.contains(&2));
...
#[test]
fn set_create_from_slice_empty() {
    let dst = BTreeEconomicSet::<i32>::create_from_slice(&[]);
    assert_eq!(dst.size(), 0);
    assert!(dst.is_empty());
}

#[test]
fn set_create_with_strategy_does_not_panic() {
    // Java: `EconomicSet.create(Equivalence)` delegates to
    // `EconomicMapImpl.create(strategy)`. `UnmodifiableEconomicSet` (and
    // `EconomicSet`) do NOT expose `getEquivalenceStrategy` — only
    // `UnmodifiableEconomicMap` does — so the strategy is only verified here
    // by exercising the construction path without panicking.
    let _set = BTreeEconomicSet::<i32>::create_with_strategy(Equivalence::IDENTITY);
    let _set = BTreeEconomicSet::<i32>::create_with_strategy(Equivalence::DEFAULT);
    let _set =
        BTreeEconomicSet::<i32>::create_with_strategy(Equivalence::IDENTITY_WITH_SYSTEM_HASHCODE);
}
