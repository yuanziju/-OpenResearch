...
//
// Rust mirror of `jdk.graal.compiler.graph.NodeSuccessorList`. Faithful 1:1 port.

use crate::graph::node::NodeId;
use crate::graph::node_list::NodeList;
use std::fmt::Debug;

/// Corresponds to `public final class NodeSuccessorList<T extends Node> extends NodeList<T>`.
///
/// A list of successor nodes. Unlike `NodeInputList`, successor list entries
/// do NOT participate in usage tracking.
///
/// Java final class → Rust struct+impl.
#[derive(Debug, Clone)]
pub struct NodeSuccessorList {
...
impl NodeSuccessorList {
    /// Corresponds to `NodeSuccessorList(Node node, int size)`.
    ...
    /// Corresponds to `NodeSuccessorList(Node node, T[] nodes)`.
    ...
    /// Corresponds to `NodeSuccessorList(Node node, List<T> nodes)`.
...
impl NodeList for NodeSuccessorList {
    fn count(&self) -> usize {
        self.node_ids.len()
    }
