...
//
// Rust mirror of `jdk.graal.compiler.debug.TimerKey`.

use crate::debug::key_registry::KeyRegistry;

/// Mirrors `jdk.graal.compiler.debug.DebugContext.TimerKey`.
/// An abstract key for a named timer metric.
#[derive(Clone, Debug)]
pub struct TimerKey {
    /// The name of this timer. Mirrors `AbstractKey.name`.
    name: String,
...
impl TimerKey {
    /// Creates a new TimerKey and registers it in the given registry.
...
impl std::hash::Hash for TimerKey {
